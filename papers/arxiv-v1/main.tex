...
official \texttt{session\_recall@8}), our retrieval harness scores \textbf{96.8\%}
(484/500), competitive with published hybrid memory systems on the same benchmark.
...
\begin{itemize}
  ...
  \textbf{96.8\%} session recall@8 on full LongMemEval-S, parity with a tuned
...
\texttt{benchmarks/results/paper-2026-07-03.json} (LoCoMo, BEIR, FAMB from
2025-06-22; LongMemEval from 2026-07-03).
...
\subsection{LongMemEval-S: 96.8\% session recall@8 on the full set}
...
sessions. This is the fair bar for comparing memory \emph{indexes} before any
reader LLM is invoked; end-to-end QA with an LLM judge is a separate, higher
layer (TiMem reports $\sim$77\% LLJ on LongMemEval-S; PlugMem $\sim$90\% Acc).

We run retrieval-only with \texttt{connect\_index()}, one engram per chat session
(LongMemEval ``value'' granularity), hybrid BM25 + dense recall, dual-key user
indexing (CP2), and heuristic query expansion (CP3). Embeddings use
\texttt{multi-qa-mpnet-base-dot-v1} on GPU (Google Colab); the full 500-question
run completes in $\sim$53 minutes ($6.4$\,s/question).
...
\midrule
FluctlightDB (full) & \textbf{96.8\%} & 484/500; hybrid + dual-key + expand \\
...
\quad single-session-preference & 76.7\% & implicit prefs; remaining gap \\
\midrule
...
a claim of strict SOTA. The headline---96.8\% on all 500 questions with a
reproducible open harness---places FluctlightDB alongside published hybrid memory
systems. The main residual error is \texttt{single-session-preference} (23/30
correct): questions ask for recommendations without naming prior facts (e.g.\
``documentary recommendations?'' when the user previously watched \emph{Our
Planet}); bridging implicit preference to the right session is the active
research frontier. Reproduce via \texttt{benchmarks/longmemeval\_colab.ipynb} or
\texttt{benchmarks/longmemeval\_bench.py} with \texttt{--granularity session
--metric session --dual-key --query-expand}.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{../figures/03-longmemeval-by-type.pdf}
\caption{LongMemEval-S session recall@8 by question type. Preference questions
(76.7\%) remain the main gap---implicit user context not named in the query.}
...
\textbf{Limitations.} LongMemEval preference questions (76.7\%) and a handful of
temporal misses remain; LLM-based key expansion (LongMemEval CP2) is not yet in
the default ingest path. We have not yet evaluated multi-tenant isolation at scale,
ANN beyond $10^5$ memories, or full end-to-end LLM QA against published
Mem0/Zep baselines on LongMemEval (their numbers use reader-LLM judges, not
session recall). These are the natural next steps.
...
FluctlightDB & Engine & hybrid index + session keys & \textbf{96.8\%} & LongMemEval session@8 \\
\bottomrule
\end{tabular}
\end{table}
...
matter, recalls 98.1\% of gold evidence on LoCoMo and 96.8\% session recall on
...
Frozen metrics: \texttt{benchmarks/results/paper-2026-07-03.json}. Preprint DOI:
