...
2025-06-22; LongMemEval unified v4 from 2026-07-04).
...
(\texttt{benchmarks/longmemeval\_e2e.py}; Colab profile \texttt{v2} with
\texttt{E2E\_LIMIT=500}). Retrieved sessions are formatted as JSON chat history;
the reader and judge use \textbf{Gemini 2.5 Flash} via the Google AI API (1M-token
context; sufficient for our top-8 session payloads). Baselines in
Table~\ref{tab:longmemeval-e2e} use \texttt{gpt-4o} or vendor-reported readers; we report our retrieval and E2E numbers together without claiming a
like-for-like SOTA comparison.
...
\midrule
FluctlightDB (v4) & \textbf{97.6\%} & (pending) & Gemini 2.5 Flash reader + judge \\
...
FluctlightDB's end-to-end score is produced by the open harness above; when the
500-question run completes, freeze JSON at
\texttt{benchmarks/results/longmemeval-e2e-v4-mpnet.json} and run
\texttt{scripts/finalize-paper-v2-e2e.sh}. We report retrieval and QA in one
table because a 97.6\% retrieval layer that feeds a weak reader still fails in
production---both numbers matter.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{../figures/03-longmemeval-by-type.pdf}
\caption{LongMemEval-S session recall@8 by question type (v4 unified 488/500).
...
matter, recalls 98.1\% of gold evidence on LoCoMo and 97.6\% session recall on
