...
(\texttt{chorus\_imprint\_batch}, GRG resonance recall) scores nDCG@10
\textbf{0.642} versus Chroma \textbf{0.645} with 2.2\,ms/doc imprint on full
SciFact. On FAMB, index macro is \textbf{100\%} and agent macro \textbf{98\%}
...
\begin{itemize}
  ...
  BEIR SciFact CHORUS competitive with Chroma on shared embeddings, and
  \textbf{100\%}/\textbf{98\%} FAMB macro (index/agent) on July 2026 reruns.
...
\texttt{benchmarks/results/paper-2026-07-08.json} (LoCoMo, BEIR, FAMB reruns
2026-07-08; LongMemEval unified v4 retrieval from 2026-07-04; E2E QA from
2026-07-07).
...
CHORUS Lane nDCG@10; FAMB reports index-mode macro score.}
\label{fig:benchmarks}
\end{figure}

\subsection{BEIR SciFact: CHORUS Lane near parity with Chroma}
We score standard IR metrics via \texttt{pytrec\_eval} against official qrels,
using identical embeddings for FluctlightDB and Chroma. The legacy index path
(\texttt{connect\_index()}) is a fast IR baseline; the **CHORUS Lane**
(\texttt{connect\_chorus()}, batch \texttt{chorus\_imprint\_batch}, GRG resonance
recall) is our bulk agent-scale ingest path---11.6\,s to imprint all 5{,}183
SciFact documents vs.\ hours for per-doc episodic \texttt{experience()}.
...
FluctlightDB (CHORUS) & \textbf{0.642} & \textbf{0.777} & 0.912 & 3 \\
\bottomrule
\end{tabular}
\end{table}

CHORUS reaches within 0.3 nDCG points of Chroma with 2.2\,ms/doc imprint and
3\,ms p50 query latency on the full corpus. Reproduce:
...
\textbf{100\%}; agent-mode macro is \textbf{98\%} (July 2026 certified rerun).
