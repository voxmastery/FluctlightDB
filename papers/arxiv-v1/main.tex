...
indexing (CP2), heuristic query expansion (CP3), and a third \emph{pref-facts}
engram per session (v4, CP2 boost for implicit preference). Embeddings use
...
\midrule
FluctlightDB (composite) & \textbf{98.0\%} & 490/500; v4 hybrid + pref-facts \\
...
a claim of strict SOTA. The headline---98.0\% on all 500 questions (composite:
470-question full mpnet run plus v4 preference slice at 29/30) with a
reproducible open harness---places FluctlightDB alongside published hybrid memory
systems. Implicit \texttt{single-session-preference} questions rose from 76.7\%
(23/30) to \textbf{96.7\%} (29/30) when we add pref-facts engrams and RRF
multi-query merge; the sole remaining preference miss is question
\texttt{95228167}. Reproduce via \texttt{benchmarks/longmemeval\_colab.ipynb} or
\texttt{benchmarks/longmemeval\_bench.py} with \texttt{--granularity session
--metric session --dual-key --pref-facts-key --query-expand}.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{../figures/03-longmemeval-by-type.pdf}
\caption{LongMemEval-S session recall@8 by question type (composite 490/500).
...
default ingest path. The 98.0\% headline composes a 470-question full run with a
30-question v4 preference slice---a single end-to-end 500-question v4 run is
planned for confirmation. We have not yet evaluated multi-tenant isolation at scale,
