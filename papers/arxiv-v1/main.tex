...
1{,}982 gold evidence spans), FluctlightDB's **CHORUS Lane** recalls \textbf{99.0\%}
...
On BEIR SciFact (shared MiniLM embeddings) the **CHORUS Lane** with **PRISM**
(RaBitQ + QJL rank, SPECTRUM certify, float32 rerank) scores nDCG@10
\textbf{0.645}, matching Chroma on the same embeddings, with 2.6\,ms/doc imprint on full
SciFact. On FAMB, \texttt{connect\_agent()} and CHORUS each score \textbf{100\%} macro
(July 2026 certified rerun). The engine, harnesses, and frozen metrics
...
\begin{itemize}
  ...
  \textbf{97.4\%} end-to-end QA on LongMemEval-S (500 questions, official judge, Muon),
  BEIR SciFact CHORUS matches Chroma nDCG@10 on shared embeddings, and
  \textbf{100\%}/\textbf{100\%} FAMB macro (agent/CHORUS) on July 2026 reruns.
...
CHORUS Lane nDCG@10; FAMB reports \texttt{connect\_agent()} macro score.}
...
using identical embeddings for FluctlightDB and Chroma. The **CHORUS Lane**
(\texttt{connect\_chorus()}, batch \texttt{chorus\_imprint\_batch}, **PRISM** readout:
RaBitQ + QJL rank, SPECTRUM certify, float32 rerank) is our bulk ingest path---13.5\,s
to imprint all 5{,}183 SciFact documents.

\begin{table}[h]
\centering
\caption{BEIR SciFact (shared MiniLM embeddings, July 2026 certified rerun).}
\begin{tabular}{lrrrr}
\toprule
...
CHORUS/PRISM matches Chroma nDCG@10 on shared embeddings with 2.6\,ms/doc imprint.
Reproduce:
...
certify cap), GRG recall with shared MiniLM vectors. The July 2026 certified rerun (embed cache warm) scores \textbf{99.0\%} mean evidence
recall.
...
(near-duplicates do not block new facts), and determinism. \texttt{connect\_agent()} macro is
\textbf{100\%}; CHORUS macro is \textbf{100\%} (July 2026 certified rerun).
...
matter, recalls 99.0\% of gold evidence on LoCoMo, 97.6\% session recall on
