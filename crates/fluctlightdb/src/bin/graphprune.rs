...
fn main() {
    let dir = PathBuf::from(
        std::env::args()
            .nth(1)
            .expect("usage: graphprune <brain-dir> [target]"),
    );
    ...
    if target >= 1000 {
        ...
        let above: usize = brain
            .graph
            .synapses
            .iter()
            .filter(|s| s.weight > thr)
            .count();
        ...
        brain.graph.synapses.retain(|s| {
            if s.weight > thr {
                true
            } else if s.weight == thr && at_budget > 0 {
                at_budget -= 1;
                true
            } else {
                false
    ...
    if target < 1000 {
        ...
        println!(
            "pruned {pruned} below {thr:.3} in {:?}; now {}",
            t1.elapsed(),
            brain.graph.synapse_count()
        );
        let t2 = std::time::Instant::now();
